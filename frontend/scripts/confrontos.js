...
function formatarDataJogo(dataIso) {
...
function renderizarConfrontos(container, partidas) {
    ...
    partidas.forEach(jogo => {
        ...
        const dataFormatada = formatarDataJogo(jogo.data_jogo);
        let seloStatus = '';
        ...
        if (jogo.status === 'FIN') {
            ...
            seloStatus = `<span class="text-xs font-semibold bg-blue-950/60 text-blue-400 border border-blue-900/50 px-2 py-0.5 rounded">${dataFormatada || 'Agendado'}</span>`;
        }

        let textoStatusDetalhe = 'Aguardando Início';
        if (jogo.status === 'FIN') {
            textoStatusDetalhe = 'Finalizado';
        } else if (jogo.status === '3') {
            textoStatusDetalhe = 'Em Andamento';
        }
        ...
        const nomeEstadio = jogo.estadio_nome || jogo.estadio || 'A Definir';

        const placarOuVs = jogo.status === '1'
            ? 'VS'
        ...
        html += `
            <div class="bg-[#161b22] border border-[#30363d] rounded-xl p-4 hover:border-gray-500 transition-all shadow-sm relative flex flex-col justify-between">
                ...
                <div class="flex items-center justify-between text-center my-4 space-x-2">
                    ...
                    <div class="w-3/12 flex items-center justify-center shrink-0">
                        <span class="text-3xl font-black text-blue-500 whitespace-nowrap">${placarOuVs}</span>
                    </div>
                    <div class="w-5/12">
                ...
                <div class="bg-[#0d1117] border border-[#30363d] rounded-xl p-4 text-left space-y-2.5 text-xs text-gray-400">
                    <div class="flex justify-between"><span class="font-semibold text-gray-500">📍 Estádio:</span> <span class="text-white font-medium text-right truncate max-w-[180px]">${nomeEstadio}</span></div>
                    <div class="flex justify-between"><span class="font-semibold text-gray-500">⏱️ Status:</span> <span class="text-white font-medium text-right">${textoStatusDetalhe}</span></div>
                </div>
            </div>
        `;
    });
